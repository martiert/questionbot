...
class Server:
    def __init__(self, config, loop):
        ...
        self._api = aiosparkapi.AioSparkApi(access_token=config['token'])
        self._callbacks = []
        self._hooks = {}
        self._get_routes = {}
        self._post_routes = {}
        ...
        self._access_restriction_check = dummy
        self._messages = []
    ...
    def access_restriction(self, callback):
        self._access_restriction_check = callback
    ...
    async def setup(self, loop):
        ...
        return await self._setup_webserver(loop)
    ...
    async def _handle_message(self, message):
        ...
        if not await self._access_restriction_check(self._api, message):
            return

        await self._pre_message(self._api, message)
    ...
    async def _message_created(self, webhook_data):
        ...
        message = await self._api.messages.get(webhook_data['data']['id'])
    ...
    async def _room_created(self, webhook_data):
        ...
        person = await self._api.people.get(webhook_data['actorId'])
    ...
    async def _setup_webserver(self, loop):
    ...
    async def _create_webhook(self, name, resource, event, callback):
        ...
        await self._api.webhooks.create(
                name,
    ...
    async def _remove_webhooks(self):
        hooks = await self._api.webhooks.list()
        async for hook in hooks:
            await self._api.webhooks.delete(hook.id)
